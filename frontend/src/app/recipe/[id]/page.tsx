...
export default function Page() {
  ...
  return (
    <Grid container size={12}>
      ...
      {meal && <Grid container spacing={4} size={12}>
          <Grid size={{md: 6, xs: 12}}>
              <Box justifySelf={'center'}>
                {meal?.strMealThumb ? <Image height={300} width={300} alt={'Meal image'} src={meal.strMealThumb}/> :
                  <ImageNotSupportedIcon/>}
              </Box>
          </Grid>
          <Grid size={{md: 5, xs: 12}}>
              <Stack>
                  <Typography textAlign={'center'} variant={'h2'}>{meal?.strMeal}</Typography>
                  <Typography sx={listItemStyle} textAlign={'center'} component={Link}
                              href={`/?country=${meal?.strArea}`}>{meal?.strArea}</Typography>
              </Stack>
          </Grid>
          ...
          <Grid size={{md: 12, xs: 12}}>
              ...
              <List sx={{border: '1px gray solid', width: '50%'}}>
                {meal && Object.entries(meal).map(([key, value], i: number) => {
